...
A FastAPI agent loop that streams its steps over SSE and resumes after a dropped connection, with run state in PostgreSQL so more than one replica serves one run. Deployed to Azure Container Apps by GitHub Actions. The spec is `docs/spec.md`; the build order and tonight's plan are `docs/build-brief.md`. Read both before doing anything.

## How to work here

- One task at a time from the build order in the spec. Each task ends with its test green. Write the failing test first.
...
- Nothing from Surefire Systems, its codebase, customers or tickets appears in this repo, in any form. This is a personal project.
- No paid cloud resource without Thomas saying yes in the conversation. Target is $0 a month idle: Container Apps free grant, Supabase free plan, GitHub free tier. No custom domain, no paid Postgres, no Redis, no static IP.
