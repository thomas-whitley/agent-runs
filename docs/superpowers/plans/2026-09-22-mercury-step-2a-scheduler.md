...
- **The claim endpoints** (`POST /checks/claim`, `/heartbeat`, `/result`) are step 2b, for the self hosted Lighthouse worker. Nothing in this plan blocks them: `site_check` runs this plan creates are always terminal (`succeeded` or `failed`) by the time `run_due_checks` returns, so 2b's claim logic never sees them.
